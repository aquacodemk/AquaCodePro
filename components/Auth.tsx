...
const Auth: React.FC<AuthProps> = ({ onAuthenticated }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-slate-950 px-6 overflow-hidden text-center">
      ...
      <div className="relative z-10 w-full max-w-sm flex flex-col items-center">
        {/* ЛОГО - Повикување директно од public */}
        <div className="w-32 h-32 md:w-40 md:h-40 bg-white rounded-[2.5rem] p-1 shadow-2xl mb-8 border-4 border-cyan-500/20 overflow-hidden flex items-center justify-center">
          {!logoError ? (
            <img 
              ...
              onError={() => {
                console.warn("Logo failed to load from root, checking base path...");
                setLogoError(true);
              }}
            />
          ) : (
        ...
        <p className="text-cyan-400 font-black text-lg md:text-xl tracking-[0.25em] uppercase mb-12">
          ватерполо тренинг
        </p>
